...
ggplot(streetrx, aes(x = state, y = log(ppm))) + 
  ...
  labs(title = "Relationship between state and log(ppm)", 
       x = "State") + 
  theme(legend.position = "none", 
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
...
$y_{is} = \mu + \alpha_{s} + \beta_1mgstr_{is} + \beta_2bulkp_{is} + \beta_3source_{is} + \epsilon_{is}$

$\alpha_s \sim Normal(\mu, \tau^2)$

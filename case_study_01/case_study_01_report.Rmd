...
- should we move the Cooks distance to the appendix?
  - discussion of qq plot is needed
- should we interpret the Bayesian model?
  - Current bayesian model does not use the results from the frequentist model, does it?
  - Model building section - only include the ex_result_int table? 
  - finish data cleaning section writing
- finish model building section writing
...
In this section, we expect to build a model that explains what factors affect the drug price and accounts for the uncertainty in the model. To simplify the modelling process while still being easy to interpret uncertainty, we decide to use a naive bayesian approach of model selection. The process has two phases. First, we run an exhaustive search on a bunch of models, and select the one that has minimum BIC. Then, we run a Bayesian hierarchical model to estimate the parameters, including the coefficients and the variances.
...
Fitting the model with frequenst MLE, we have the following results.
```{r}
...
Now we impose a prior on the parameters and fit the model using a Bayesian approach. Since we do not have much information about the model, we use non-informative priors:

...
We find that No_setts_in_fields is the most important variable with respect to our outcome, Signs_in_yard. 
...
* For a 1 unit increase in the number of setts in the field, we expect the odds of the presence of signs in the farmyard to increase by 45% (exp(0.37) = 1.45).
* Intercept: In the absence of any settlements in the field, we expect the odds of the presence of signs in the farmyard to decrease by 99% (exp(-5.03) = 0.007).
...
# dotplot
library(lattice)
dotplot(ranef(m1, condVar = TRUE), font.size = 5, rotate = TRUE)$farm_code_numeric
```
...
The 3rd research question asks us to estimate the correlation over time in badger activity, and we thus examine the ICC. We find that our ICC is ~0.6, which measures the correlation of observations within each farm. This also accounts for correlation over time in badger activity, as each farm's measurements were taken at a different point in time. Since our ICC is relatively high, we find that badger activity is relatively correlated over time. 
...
# BIC 172.2    
m2 <- glmer(Signs_in_yard ~ No_setts_in_fields + No_active_setts_in_fields +
              (1 | farm_code_numeric), data = badgers, 
      family = binomial(link = "logit"), 
      control = glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 2e5)))
summary(m2)
...
# BIC 171.2
m3 <- glmer(Signs_in_yard ~ No_setts_in_fields + No_cattle_in_buidlings_yard +
              (1 | farm_code_numeric), data = badgers, 
      family = binomial(link = "logit"), 
      control = glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 2e5)))
summary(m3)
...
# BIC 173.0    
m4 <- glmer(Signs_in_yard ~ No_setts_in_fields + Accessible_feed_store_present +
              (1 | farm_code_numeric), data = badgers, 
      family = binomial(link = "logit"), 
      control = glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 2e5)))
summary(m4)
...
# BIC 172.3
m5 <- glmer(Signs_in_yard ~ No_setts_in_fields + HayStraw +
              (1 | farm_code_numeric), data = badgers, 
      family = binomial(link = "logit"), 
      control = glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 2e5)))
summary(m5)
...
# BIC 173.1
m6 <- glmer(Signs_in_yard ~ No_setts_in_fields + Concentrates +
              (1 | farm_code_numeric), data = badgers, 
      family = binomial(link = "logit"), 
      control = glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 2e5)))
summary(m6)
...
# BIC 172.4
m7 <- glmer(Signs_in_yard ~ No_setts_in_fields + Sugarbeet +
              (1 | farm_code_numeric), data = badgers, 
      family = binomial(link = "logit"), 
      control = glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 2e5)))
summary(m7)
...
# BIC 173.1 
m8 <- glmer(Signs_in_yard ~ No_setts_in_fields + Molasses +
              (1 | farm_code_numeric), data = badgers, 
      family = binomial(link = "logit"), 
      control = glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 2e5)))
summary(m8)

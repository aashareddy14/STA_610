...
Our first research question asks us to examine factors related to the presence of badger activity in the farmyard. Through step-wise selection using BIC, we find that the model with only `No_active_setts_in_fields` led to the lowest BIC. The below table shows the estimates of the fixed effects on a logit scale, which we can interpret on the odds by exponentiating as follows: 
	...
	* For a 1 unit increase in the number of active settlements in the field, we expect the odds of the presence of signs of badgers in the farmyard to increase by 63% (exp(0.49) = 1.63).
	* Intercept: In the absence of any settlements in the field, we expect the odds of the presence of signs of badgers in the farmyard to decrease by 99% (exp(-4.87) = 0.007).
...
$$
ICC = \frac{\sigma^2}{\sigma^2 + \frac{\pi^2}{3}} = 0.64
$$
